...
std::shared_ptr<spdlog::logger>& GetLogger()
{
	if (psetLogger.get() == nullptr)
	{
		...
		logSinks.emplace_back(std::make_shared<spdlog::sinks::basic_file_sink_mt>("fgac.log", true));

		logSinks[0]->set_pattern("%^[%T] %n: %v%$");
		logSinks[1]->set_pattern("[%T] [%l] %n: %v");

		psetLogger = std::make_shared<spdlog::logger>("FGAC", begin(logSinks), end(logSinks));
