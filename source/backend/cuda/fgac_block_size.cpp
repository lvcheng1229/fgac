...
const quant_and_transfer_table quant_and_xfer_tables_def[12]{
	// QUANT2, range 0..1
	{
		{0, 64},
		{0, 1},
...
void prepare_quant_and_xfer_tables(block_size_descriptor& bsd)
{
	memcpy(bsd.quant_and_xfer_tables, quant_and_xfer_tables_def,sizeof(quant_and_transfer_table) * 12);
}
